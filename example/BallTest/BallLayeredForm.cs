...
namespace BallTest
{
    public class BallLayeredForm : ILayeredFormOpacity
    {
        #region 属性
        ...
        /// 菜单项列表 - 使用AntdUI控件
        /// </summary>
        private IContextMenuStripItem[] menuItems;
        ...
        // 初始化菜单项
        private IContextMenuStripItem[] InitializeMenuItems()
        {
            ...
            {
                new ContextMenuStripItem("返回", "Alt+向左键").SetIcon(svg_back),
                new ContextMenuStripItem("刷新", "Ctrl+R").SetIcon(svg_refresh),
                new ContextMenuStripItemDivider(),
                new ContextMenuStripItem("另存为", "Ctrl+S").SetIcon(svg_save),
                new ContextMenuStripItem("打印", "Ctrl+P").SetIcon(svg_print),
                new ContextMenuStripItemDivider(),
                new ContextMenuStripItem("发送标签页到你的设备").SetIcon(svg_laptop),
                new ContextMenuStripItem("为此页面创建QR代码").SetIcon(svg_qr),
                new ContextMenuStripItem("大声朗读", "Ctrl+Shift+U").SetIcon(svg_a),
                new ContextMenuStripItem("翻译为 中文(简体)").SetIcon(svg_fy),
                new ContextMenuStripItem("将页面添加到集锦").SetIcon(svg_add).SetSub(
                    new ContextMenuStripItem("返回", "Alt+向左键").SetIcon(svg_back).SetSub(
                        new ContextMenuStripItem("返回", "Alt+向左键").SetIcon(svg_back),
                        new ContextMenuStripItem("刷新", "Ctrl+R").SetIcon(svg_refresh),
                        new ContextMenuStripItemDivider(),
                        new ContextMenuStripItem("另存为", "Ctrl+S").SetIcon(svg_save),
                ...
                new ContextMenuStripItemDivider(),
                new ContextMenuStripItem("共享").SetIcon(svg_share),
                new ContextMenuStripItem("关于").SetIcon(svg_about)
            };
        }
        ...
        public override Bitmap? PrintBit()
        {
            ...
            using (var g = Graphics.FromImage(bitmap).HighLay())
            {
                ...
                int radius = Math.Min(centerX, centerY) - bor;
                ...
                {
                    // 绘制圆形主体
                    g.FillEllipse(gradientBrush, new Rectangle(centerX - radius, centerY - radius, radius * 2, radius * 2));
                    ...
                    using (var highlightBrush1 = new SolidBrush(Color.FromArgb(_highlightAlpha1, _highlightColor1)))
                    {
                        ...
                        g.FillEllipse(highlightBrush1, new Rectangle(
                    ...
                    using (var highlightBrush2 = new SolidBrush(Color.FromArgb(_highlightAlpha2, _highlightColor2)))
                    {
                        ...
                        g.FillEllipse(highlightBrush2, new Rectangle(
                    ...
                    // 调整边框：使用深蓝色边框，更精致
                    using (var pen = new Pen(Color.FromArgb(_borderAlpha, _borderColor), bor))
                    {
                        g.DrawEllipse(pen, new Rectangle(centerX - radius, centerY - radius, radius * 2, radius * 2));
                    }
                }
            }
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            {
                int deltaX = e.X - dragStartPoint.Value.X, deltaY = e.Y - dragStartPoint.Value.Y;
                ...
                if (isBeingDragged)
                {
                    // 更新位置
                    var rect = TargetRect;
                    int newX = rect.X + deltaX, newY = rect.Y + deltaY;
                    // 设置新位置
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            ...
            {
                // 左键点击关闭窗口
                if (e.Button == MouseButtons.Left) IClose();
            }
