...
namespace AntdUI
{
    internal class LayeredFormSelectMultiple : ISelectMultiple
    {
        ...
        ObjectItem ItemC(object value, int i, ref int item_count, ref int divider_count, ref int y, int padd, int padd2, int sp, int gap_x, int gap_x2, int icon_size, int icon_gap, int icon_xy, int item_height, int text_height, int maxwr, ref int sy, bool no_id = false)
        {
            ObjectItem item;
        ...
        void OnClick(ObjectItem it)
        {
            ...
            else if (selectedValue.Contains(ReadValue(it.Item))) selectedValue.Remove(ReadValue(it.Item));
            else
            {
