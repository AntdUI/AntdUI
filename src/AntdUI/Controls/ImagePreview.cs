...
namespace AntdUI
{
    ...
    public class ImagePreview : IControl
    {
        ...
        void LoadGif()
        {
            ...
            Helper.GIFPlay(imgtmp, value => PlayGIF && imgtmp == value, Invalidate);
        }
        ...
        void PaintLoading(Canvas g, Rectangle rect, bool error = false)
        {
            ...
            if (error)
            {
                bg = Colour.Error.Get(nameof(Preview), Name);
                color = Colour.ErrorColor.Get(nameof(Preview), Name);
            }
            else
            {
                bg = Colour.Primary.Get(nameof(Preview), Name);
                color = Colour.PrimaryColor.Get(nameof(Preview), Name);
            }
