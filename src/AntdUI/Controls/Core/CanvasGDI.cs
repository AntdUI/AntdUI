    ...
    public class CanvasGDI : Canvas
    {
        ...
        public Size MeasureText(string? text, Font font, int width, StringFormat format)
        {
            ...
            else
            {
                ...
                {
                    if (Config.EmojiEnabled && GraphemeSplitter.IsEmoji(ntype, txt))
                    {
        ...
        {
            using (var cache = new TFixFont())
            {
                int h = cache.Height(this, font);
                // 遍历每个字符，计算宽度
                foreach (var it in characters)
                {
                    if (it.emoji) it.w = h;
                    else if (it.txt == "\r" || it.txt == "\n" || it.txt == "\r\n") it.w = 0;
                    else it.w = cache.Width(this, font, it.txt);
                }
                return h;
            }
        }
