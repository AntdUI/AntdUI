        ...
        public class StyleCard2 : IStyle
        {
                            ...
                            case TabAlignment.Bottom:
                                ...
                                foreach (var it in rect_dir)
                                {
                                    if (it.Key.Visible)
                                    {
                                        ...
                                        bool isReadOnly = it.Key.ReadOnly;
                                        ...
                                        if (hasIcon && width == 0)
                                        {
                                            ...
                                            if (!isReadOnly) width += close_size + ico_gap;
                                        }
                                        else
                                        {
                                            if (hasIcon) width += ico_size + ico_gap * 2;
                                            if (!isReadOnly) width += close_size + ico_gap;
                                        }
                                        ...
                                        if (isReadOnly)
                                        {
                                            if (hasIcon) rect_list.Add(new TabPageRect(rect_it, it.Value, gap, ico_size, ico_gap, true));
                                            else rect_list.Add(new TabPageRect(rect_it, it.Value, gap, true));
                                        }
                                        else
                                        {
                                            if (hasIcon) rect_list.Add(new TabPageRect(rect_it, it.Value, gap, ico_size, close_size, ico_gap, true));
                            ...
                            case TabAlignment.Bottom:
                                ...
                                foreach (var it in rect_dir)
                                {
                                    if (it.Key.Visible)
                                    {
                                        ...
                                        if (hasIcon && width == 0) width = ico_size + ico_gap;
                                        else
                                        {
                                            if (hasIcon) width += ico_size + ico_gap * 2;
                                        }
                            ...
                            default:
                                foreach (var it in rect_dir)
                                {
                                    if (it.Key.Visible)
                                    {
                                        ...
                                        if (hasIcon) rect_list.Add(new TabPageRect(rect_it, it.Value, gap, ico_size, ico_gap, true, rw_tmp));
                                        else rect_list.Add(new TabPageRect(rect_it, it.Value, gap, true, rw_tmp));
                                        it.Key.SetRect(rect_it);
                                        xy += rect_it.Width + cardgap;
                                    }
            ...
            public void Paint(Tabs owner, Canvas g, TabCollection items)
            {
                if (rects.Length == items.Count)
                {
                    ...
                    using (var brush_bg = new SolidBrush(Fill ?? Colour.FillQuaternary.Get(owner.ColorScheme, nameof(Tabs), owner.Name)))
                    using (var brush_bg_hover = new SolidBrush(FillHover ?? Colour.FillQuaternary.Get(owner.ColorScheme, nameof(Tabs), owner.Name)))
                    using (var brush_bg_active = new SolidBrush(FillActive ?? Colour.BgContainer.Get(owner.ColorScheme, nameof(Tabs), owner.Name)))
                    {
                                    ...
                                    if (page.Visible)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            using (var path = Helper.RoundPath(page.Rect, radius, true, true, true, true))
                                            {
                                                ...
                                                if (bor > 0) g.Draw(border ?? Colour.BorderSecondary.Get(owner.ColorScheme, nameof(Tabs), owner.Name), bor, path);
                                ...
                                if (sel != null)//是否选中
                                {
                                    ...
                                    using (var path = Helper.RoundPath(rect_page, radius, true, true, true, true))
                                    {
                                        if (bor > 0)
                                        {
                                            ...
                                            g.Draw(BorderActive ?? Colour.BorderColor.Get(owner.ColorScheme, nameof(Tabs), owner.Name), bor, path);
                                        }
                                        else
                                        {
                                            if (owner.scroll_show) g.TranslateTransform(-owner.scroll_x, -owner.scroll_y);
